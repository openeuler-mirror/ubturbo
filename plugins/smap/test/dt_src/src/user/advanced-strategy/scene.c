...
#include "scene.h"
 ...
 bool g_adaptLocalMem = true;

  int PrevIndex(int index, int len)
{
    return (index + len - 1) % len;
}

  int NextIndex(int index, int len)
{
    return (index + 1) % len;
}

  void IncPageIndex(SceneInfo *info)
{
    int next;
 ...
 int GetDeltaHot(SceneInfo *info, int currIdx)
{
    int prevIdx;
    int deltaHot;
 ...
 bool TryFromStableToUnstable(SceneInfo *info, int idx)
{
 ...
 bool TryToStayInUnstable(SceneInfo *info, int idx)
{
    PageInfo *p = &info->pageInfo[idx];
    int deltaHot;
    deltaHot = GetDeltaHot(info, idx);
 ...
 bool IsUnstableScene(SceneInfo *info)
{
    int i;
    int currIdx;
 ...
 bool IsHeavyLoadScene(SceneInfo *info)
{
    int i, idx;
    PageInfo *pageinfo;
 ...
 void AnalyzeScene(SceneInfo *info)
{
 ...
 int StatsL2AvgHotPages(ProcessAttr *process)
{
 ...
 uint32_t StatsMaxGuaranteePages(ProcessAttr *process)
{
    SceneInfo *info = &process->sceneInfo;
    int idx = info->pageInfoIndex;
    uint32_t maxGua = 0;
 ...
 int StatsMaxHotPages(ProcessAttr *process)
{
    SceneInfo *info = &process->sceneInfo;
    int idx = info->pageInfoIndex;
    int maxHot = 0;
 ...
 void StatsGuaranteePages(ProcessAttr *process)
{
    SceneInfo *info = &process->sceneInfo;
 ...
 void CalcMemInfo(ProcessAttr *process)
{
...
int SetProcessSceneAttr(ProcessAttr *process)
{
 ...
 void SetLocalMemStatus(SceneInfo *info)
{
    PageInfo *p = &info->pageInfo[info->pageInfoIndex];
  ...
  int AddList(int arr[], int len)
{
 ...
 void UpdateMemRatio(ProcessAttr *process)
{
  ...
  void ClearList(int arr[], int len)
{
    for (int i = 0; i < len; i++) {
        arr[i] = 0;
    }
 ...
 void DistributeExtraPages(int arr[], int len)
{
    int i, diff;
 ...
 void DistributeInsufficientPages(int arr[], int len)
{
    int i, diff;
    double posSum = 0, negSum = 0, sum = 0;
    int consumer[len];
 ...
 void BalanceSurpluses(int arr[], int len)
{
...
void SetAdaptMem(bool flag)
{
 ...
 bool IsReadyForAdapt(ProcessAttr *attr)
{
 ...
 void AdjustVmMemRatio(struct ProcessManager *manager, int *surpluses, int len)
{
    int i = 0;
    if (len <= 0) {
        return;
...
void ConfigMultiVmRatio(struct ProcessManager *manager)
{
 ...
 void GetMaxNuma(struct ProcessManager *manager, int *maxL1node, int *maxL2node)
{
 ...
 bool ProcessMultiNumaVmNode(ProcessAttr *process)
{
 ...
 void ConfigMultiVmRatioInGroups(struct ProcessManager *manager)
{
 ...
 void SkipMultiProcessRatio(struct ProcessManager *manager)
{
    EnvMutexLock(&manager->lock);
    ProcessAttr *current = manager->processes;
    while (current) {
