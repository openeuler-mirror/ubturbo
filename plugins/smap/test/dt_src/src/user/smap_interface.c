 ...
 bool ubturbo_smap_is_running(void)
{
 ...
 int IoctlHandler(const void *msg)
{
  ...
  bool IsRatioValid(int ratio)
{
    return (ratio >= 0 && ratio <= HUNDRED);
}

 bool IsMigOutCountValid(pid_t *pidArr, int len, int pidType)
{
    int newNum = 0;
  ...
  bool IsCountValid(int count, int max)
{
    return (count > 0 && count <= max);
}

 int CheckPidtype(uint32_t pageType)
{
    int ret = 0;
    char path[PATH_MAX];
 ...
 bool IsPidTypeValid(int pidType)
{
...
/* Validate that a remote nid is in range and currently visible in numastat. */
 bool IsOnlineRemoteNidValid(int nid)
{
 ...
 bool IsPidArrValid(pid_t *pidArr, int len, bool ignoreUnmanaged)
{
    int i;
 ...
 int InitAllThreads(struct ProcessManager *manager)
{
    int ret;
 ...
 bool IsDestNidVaild(int nid, pid_t pid)
{
    ProcessAttr *attr;
 ...
 bool IsDestNidUnique(struct MigrateOutPayload *payload)
{
 ...
 bool CheckMigOutPayloadItems(struct MigrateOutPayload *payload, uint64_t *totalRatio)
{
 ...
 bool IsMigParaValid(struct MigrateOutPayload *payload)
{
    if (!payload) {
 ...
 int BuildProcessTargetConfig(const struct MigrateOutPayload *payload, ProcessTargetConfig *config)
{
 ...
 int CheckMigrateOutMsg(struct MigrateOutMsg *msg, int pidType)
{
    int i;
    if (!msg) {
 ...
 bool HasDuplicateInt(const int *arr, int count)
{
 ...
 int CheckGroupedTarget(const struct MigrationGroup *group, int payloadIdx, int groupIdx)
{
 ...
 int CheckGroupedPayload(struct GroupedMigrateOutPayload *payload, int payloadIdx)
{
 ...
 int CheckGroupedMigrateOutMsg(struct GroupedMigrateOutMsg *msg, int pidType)
{
    if (!msg) {
 ...
 void BuildGroupedNodeBitmap(const struct GroupedMigrateOutPayload *payload, uint32_t *nodeBitmap)
{
    *nodeBitmap = 0;
    for (int i = 0; i < payload->groupCount; i++) {
 ...
 int BuildGroupPolicy(const struct GroupedMigrateOutPayload *payload, const uint64_t numaPages[MAX_NODES],
                            GroupMigrationPolicy *policy)
{
 ...
 int ProcessAddGroupedTrackingManageFiltered(struct GroupedMigrateOutMsg *msg, uint32_t *nodeBitmap,
                                                   const bool *skipTracking)
{
 ...
 int ProcessAddGroupedTrackingManage(struct GroupedMigrateOutMsg *msg, uint32_t *nodeBitmap)
{
    return ProcessAddGroupedTrackingManageFiltered(msg, nodeBitmap, NULL);
}

 int BuildGroupedPolicies(struct GroupedMigrateOutMsg *msg, GroupMigrationPolicy policies[MAX_NR_GROUPED_MIGOUT])
{
    for (int i = 0; i < msg->count; i++) {
 ...
 void RollbackGroupedManagerAdds(const pid_t *addedPids, int addedCnt);
 void RollbackGroupedTrackingManage(struct GroupedMigrateOutMsg *msg, const bool *keepTracking);
 int AddGroupedProcessesToGlobalManagerFiltered(struct GroupedMigrateOutMsg *msg, uint32_t *nodeBitmap,
                                                      GroupMigrationPolicy policies[MAX_NR_GROUPED_MIGOUT],
                                                      bool keepTracking[MAX_NR_GROUPED_MIGOUT],
                                                      const bool *pendingUpdate);

 int AddGroupedProcessesToGlobalManager(struct GroupedMigrateOutMsg *msg, uint32_t *nodeBitmap,
                                              GroupMigrationPolicy policies[MAX_NR_GROUPED_MIGOUT],
                                              bool keepTracking[MAX_NR_GROUPED_MIGOUT])
{
 ...
 bool HasGroupedPendingUpdate(const bool *pendingUpdate, int count)
{
 ...
 int SetPendingGroupedPolicies(struct GroupedMigrateOutMsg *msg, uint32_t *nodeBitmap,
                                     GroupMigrationPolicy policies[MAX_NR_GROUPED_MIGOUT], const bool *pendingUpdate)
{
 ...
 bool IsGroupedAttrByRemoteNid(ProcessAttr *attr, int remoteNid)
{
    if (!attr || !attr->groupPolicy.enabled) {
        return false;
    }
 ...
 bool IsAnyGroupedPidOnRemoteNid(int remoteNid)
{
    struct ProcessManager *manager = GetProcessManager();
 ...
 int BuildMigrateOutProcessParamWithCapacityPolicy(const struct MigrateOutPayload *payload, ProcessParam *param,
                                                         bool ignoreRemoteCapacity)
{
 ...
 void DiscardMigrateOutCandidates(ProcessManageCandidate *candidates, int count)
{
    for (int i = 0; i < count; i++) {
 ...
 int PrepareMigrateOutCandidatesWithCapacityPolicy(struct MigrateOutMsg *msg, int pidType,
                                                         ...
                                                         bool ignoreRemoteCapacity)
{
 ...
 int PrepareMigrateOutCandidates(struct MigrateOutMsg *msg, int pidType, ProcessManageCandidate *candidates,
                                       uint32_t *nodeBitmap)
{
    return PrepareMigrateOutCandidatesWithCapacityPolicy(msg, pidType, candidates, nodeBitmap, false);
}

 int TrackMigrateOutCandidates(ProcessManageCandidate *candidates, int count)
{
 ...
 void PublishMigrateOutCandidates(ProcessManageCandidate *candidates, int count)
{
    for (int i = 0; i < count; i++) {
 ...
 int MigrateOutWithCapacityPolicy(struct MigrateOutMsg *msg, int pidType, bool ignoreRemoteCapacity)
{
    struct ProcessManager *manager = GetProcessManager();
...
int ubturbo_smap_migrate_out_grouped(struct GroupedMigrateOutMsg *msg, int pidType)
{
 ...
 int CheckMigrateBackMsg(struct MigrateBackMsg *msg)
{
 ...
 int CheckMigrateBackReadyMsg(struct MigrateBackMsg *msg)
{
    for (int i = 0; i < msg->count; i++) {
 ...
 int CheckMigrateBackGroupedPidLocked(struct MigrateBackMsg *msg)
{
    for (int i = 0; i < msg->count; i++) {
        int srcNid = msg->payload[i].srcNid;
 ...
 bool IsMigrateBackNidDuplicatedBefore(struct MigrateBackMsg *msg, int idx)
{
 ...
 void ClearMigrateBackBusyForbiddenBefore(struct MigrateBackMsg *msg, int count)
{
    for (int i = 0; i < count; i++) {
        int srcNid = msg->payload[i].srcNid;
 ...
 void ClearMigrateBackBusyForbidden(struct MigrateBackMsg *msg)
{
    ClearMigrateBackBusyForbiddenBefore(msg, msg->count);
}

 void CompleteMigrateBackForbidden(struct MigrateBackMsg *msg)
{
    for (int i = 0; i < msg->count; i++) {
        int srcNid = msg->payload[i].srcNid;
        if (IsMigrateBackNidDuplicatedBefore(msg, i)) {
 ...
 int SetMigrateBackForbiddenLocked(struct MigrateBackMsg *msg)
{
 ...
 bool CheckProcessIdle(int nid)
{
 ...
 void MarkAutoRemoveByMigrateBack(struct MigrateBackMsg *msg);
...
/* Validate remove payload shape before touching kernel or manager state. */
 int CheckSmapRemoveMsg(struct RemoveMsg *msg, int pidType)
{
...
/* A remove payload with no nid keeps the legacy whole-process remove behavior. */
 bool IsRemoveWholeProcess(const struct RemovePayload *payload)
{
    return payload->count == 0;
}

/* Remove all access-tracking state for pid from the kernel module. */
 int AccessRemovePid(pid_t pid)
{
...
/* Build the remaining remote-node mask after clearing requested remote nodes. */
 uint32_t ClearRemovePayloadRemoteNodes(ProcessAttr *attr, const struct RemovePayload *payload)
{
...
/* Refresh kernel access-tracking state with the remaining remote nodes. */
 int AccessUpdateProcessRemoteNodes(ProcessAttr *attr, uint32_t numaNodes)
{
...
/* Apply remove requests to kernel access tracking before changing manager state. */
 int IoctlRemoveProcess(struct RemoveMsg *msg)
{
    for (int i = 0; i < msg->count; i++) {
        struct RemovePayload *payload = &msg->payload[i];
...
/* Partial remove cannot safely update grouped policy state, so reject it. */
 int CheckSmapRemoveGroupedPidLocked(struct RemoveMsg *msg)
{
    for (int i = 0; i < msg->count; i++) {
        struct RemovePayload *payload = &msg->payload[i];
        if (IsRemoveWholeProcess(payload)) {
...
/* Remove one pid from the user-space manager process list. */
 void ClearManagedWholeProcess(pid_t pid, bool *removed)
{
    struct ProcessManager *manager = GetProcessManager();
...
/* Clear strategy/accounting fields that belong to the removed remote node. */
 void ClearRemovedRemoteStrategy(ProcessAttr *attr, int remoteNid)
{
    int nrLocalNuma = GetNrLocalNuma();
...
/* Drop one remote node from a managed process and compact migrateParam. */
 void RemoveManagedProcessRemoteNode(ProcessAttr *attr, int remoteNid)
{
    int nrLocalNuma = GetNrLocalNuma();
...
/* Apply a partial remote-node remove to the user-space manager state. */
 void ClearManagedPartialRemoteNodes(struct RemovePayload *payload, bool *removed, bool *changed)
{
    struct ProcessManager *manager = GetProcessManager();
    ProcessAttr *attr = manager->processes;
...
/* Keep manager state and persisted process config consistent after remove. */
 void ClearManagedProcess(int nr, struct RemovePayload *payload)
{
    bool removed = false;
    bool changed = false;
...
int ubturbo_smap_remove(struct RemoveMsg *msg, int pidType)
{
 ...
 bool HasRemotePages(ProcessAttr *attr)
{
    int nrLocalNuma = GetNrLocalNuma();
 ...
 bool IsMigrateBackSourceNid(const struct MigrateBackMsg *msg, int nid)
{
    if (!msg) {
        return false;
    }
 ...
 bool IsMigrateBackAutoRemoveCandidate(ProcessAttr *attr, const struct MigrateBackMsg *msg)
{
    if (!msg) {
        return false;
    }
 ...
 bool IsAutoRemoveCandidate(ProcessAttr *attr)
{
 ...
 void MarkAutoRemoveByMigrateBack(struct MigrateBackMsg *msg)
{
 ...
 bool IsPidAlreadyCollected(pid_t *pids, int count, pid_t pid)
{
    for (int i = 0; i < count; i++) {
        if (pids[i] == pid) {
            return true;
 ...
 int CollectRemoteEmptyAutoRemovePids(pid_t *pids, int maxCount)
{
    struct ProcessManager *manager = GetProcessManager();
    int count = 0;
 ...
 void RemoveAutoRemovePids(pid_t *pids, int count)
{
    struct ProcessManager *manager = GetProcessManager();
 ...
 int SyncProcessToKernel(void)
{
 ...
 void RecoverRemoveInvalidProcess(void)
{
    int ret;
    struct ProcessManager *manager = GetProcessManager();
...
int ubturbo_smap_remote_numa_info_set(struct SetRemoteNumaInfoMsg *msg)
{
 ...
 int CheckQueryVMFreqMsgValid(int pid, uint16_t *data, uint32_t lengthIn, uint32_t *lengthOut, int dataSource)
{
    time_t currentTime;
    struct ProcessManager *manager = GetProcessManager();
    if (!data || !lengthOut) {
 ...
 int QueryVMFreqFromKernel(int pid, uint16_t *data, uint32_t lengthIn, uint32_t *lengthOut)
{
    int ret;
    struct ProcessManager *manager = GetProcessManager();
 ...
 int QueryVMFreqFromUser(int pid, uint16_t *data, uint32_t lengthIn, uint32_t *lengthOut)
{
    uint64_t i = 0;
    uint64_t actcLen = 0;
    struct ProcessManager *manager = GetProcessManager();
...
int ubturbo_smap_run_mode_set(int runMode)
{
 ...
 int CheckAddProcessTrackingMsg(pid_t *pidArr, uint32_t *scanTime, uint32_t *duration, int len, int scanType)
{
    struct ProcessManager *pm = GetProcessManager();
 ...
 int AddProcessTracking(pid_t *pidArr, uint32_t *scanTime, uint32_t *duration, int len, int scanType)
{
    int ret = 0;
    struct ProcessManager *pm = GetProcessManager();
    int maxProcessCnt = GetCurrentMaxNrPid();
...
int ubturbo_smap_process_tracking_add(pid_t *pidArr, uint32_t *scanTime, uint32_t *duration, int len, int scanType)
{
 ...
 int CheckRemoveProcessTrackingMsg(pid_t *pidArr, int len)
{
    int ret = 0;
    struct ProcessManager *manager = GetProcessManager();
...
int ubturbo_smap_process_migrate_enable(pid_t *pidArr, int len, int enable, int flags)
{
 ...
 int CheckMigrateNumaMsg(struct MigrateNumaMsg *msg)
{
    struct ProcessManager *manager = GetProcessManager();
...
int ubturbo_smap_remote_numa_migrate(struct MigrateNumaMsg *msg)
{
 ...
 int CheckSameMigrateNumaMsg(struct MigrateNumaMsg *msg)
{
    struct ProcessManager *manager = GetProcessManager();
...
int ubturbo_smap_same_remote_numa_migrate(struct MigrateNumaMsg *msg)
{
 ...
 int CheckMigOutSyncMsg(int pidType, uint64_t maxWaitTime)
{
    ...
    if ((maxWaitTime < MIN_WAIT_TIME || maxWaitTime > MAX_WAIT_TIME) && maxWaitTime != 0) {
        SMAP_LOGGER_ERROR("The maxWaitTime parameter is improper,The maxWaitTime from 10s to 1 min_stub.");
        return -EINVAL;
    }
 ...
 bool IsMigrateOutPayloadRemoteTargetZero(const struct MigrateOutPayload *payload)
{
 ...
 void SetSyncWaitRemoteEmpty(struct MigrateOutMsg *msg, bool enable)
{
    struct ProcessManager *manager = GetProcessManager();
 ...
 int CheckMigOutSyncResult(struct MigrateOutMsg *msg, int *invalidPidNum, bool *allPidSuccess,
...
out:
 ...
 bool IsScanTypeValid(pid_t *pidArr, int len)
{
    int i;
    ProcessAttr *attr;
 ...
 * 返回值：0-是，其它-异常
 */
 int IsPidArrRemoteNumaMatch(struct MigrateEscapeMsg *msg)
{
    struct ProcessManager *manager = GetProcessManager();
    int ret = 0;
    EnvMutexLock(&manager->lock);
 ...
 int GetAttrNidInitRatio(pid_t pid, int nid)
{
    int nrLocalNuma = GetNrLocalNuma();
    ProcessAttr *attr = GetProcessAttr(pid);
    if (!attr) {
 ...
 bool IsRemoteNidRatioValid(pid_t pid, int nid, int ratio)
{
 ...
 int GetAttrNidInitMemSize(pid_t pid, int nid, uint64_t *memSize)
{
    int nrLocalNuma = GetNrLocalNuma();
 ...
 bool IsRemoteNidMemSizeValid(pid_t pid, int nid, uint64_t memSize)
{
 ...
 int SmapMigratePidRemoteNumaCheckInner(struct MigrateEscapeMsg *msg)
{
 ...
 int SmapMigratePidRemoteNumaCheck(struct MigrateEscapeMsg *msg)
{
    if (!msg) {
 ...
 int BuildMigRemoteNumaMsg(struct MigrateEscapeMsg *msg, struct MigPidRemoteNumaIoctlMsg *ioctlMsg)
{
    int i;
...
out:
 ...
 int AssignOldProcessPayload(struct OldProcessPayload *result, ProcessAttr *attr, int l2Node)
{
    int len, l2Index;
    int nrLocalNuma = GetNrLocalNuma();
    int l1Node = GetAttrL1(attr);
 ...
 int CheckSmapQueryProcessConfig(int nid, struct OldProcessPayload *result, int inLen, int *outLen)
{
    if (!IsOnlineRemoteNidValid(nid)) {
