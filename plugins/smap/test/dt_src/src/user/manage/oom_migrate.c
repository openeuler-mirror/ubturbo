...
/* move_pages(2) 薄封装（mockable seam）：UT 经 run_dt.sh strip  后拦截此处，不触达真实 syscall。 */
...
/* 解析一行 numa_maps：取段起始地址，累加本地页数 = Σ N<i>=<count>，i ∈ [0, nrLocalNuma)。 */
 bool ParseNumaMapLocalCount(const char *line, int nrLocalNuma, unsigned long *segStart, uint64_t *localCount)
{
 ...
 */
 int CollectVaddrsBatch(FILE *fp, int nrLocalNuma, uint64_t pageSize, uint64_t maxPages, NumaScanCursor *cur,
