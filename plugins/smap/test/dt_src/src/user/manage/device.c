...
typedef enum {
 ...
 int SendCmdToAllNodes(int fds[], unsigned long cmd, int arg)
{
    int i;
    int ret = 0;
 ...
 int FindFdByNode(int fds[], int fdsLength)
{
    int i;
...
int EnableTracking(struct ProcessManager *manager)
{
  ...
  int DisableTrackingInternal(struct ProcessManager *manager)
{
 ...
 int ConfigTrackingDev(int *trackingFds, uint32_t pageSize)
{
 ...
 bool IsLocalNuma(unsigned long nid)
{
...
#undef SYS_NODE_REMOTE_LEN
 ...
 int GetNrLocalNumaFromKernel(struct ProcessManager *manager)
{
    if (manager->fds.access < 0) {
        SMAP_LOGGER_ERROR("access device fd is invalid: %d", manager->fds.access);
        return -EINVAL;
