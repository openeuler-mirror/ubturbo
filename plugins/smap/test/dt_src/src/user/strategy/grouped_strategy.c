...
typedef struct {
 ...
 int CmpU64Asc(const void *a, const void *b)
{
 ...
 void SortMigListAddr(struct MigList *list)
{
    if (list->nr > 1) {
        // 对 addr 数组升序排序
 ...
 int GroupPageAscFunc(const void *data1, const void *data2)
{
 ...
 int GroupPageDescFunc(const void *data1, const void *data2)
{
    const GroupPageData *p1 = (const GroupPageData *)data1;
    const GroupPageData *p2 = (const GroupPageData *)data2;
    if (p1->isWhiteListPage != p2->isWhiteListPage) {
 ...
 int PromoteGroupCmp(const void *data1, const void *data2)
{
 ...
 bool GroupHasLocal(const MigrationGroupAttr *group, int nid)
{
 ...
 int GroupTargetIndex(const MigrationGroupAttr *group, int nid)
{
 ...
 bool PolicyHasNode(const GroupMigrationPolicy *policy, int nid)
{
 ...
 void WarnUngroupedPages(ProcessAttr *process)
{
 ...
 uint64_t CalcLocalUsed(ProcessAttr *process, const MigrationGroupAttr *group)
{
    uint64_t used = 0;
    for (int i = 0; i < group->localCount; i++) {
 ...
 uint64_t CalcLocalDeficit(ProcessAttr *process, const MigrationGroupAttr *group, int localIdx)
{
 ...
 uint64_t CalcLocalExcess(ProcessAttr *process, const MigrationGroupAttr *group, int localIdx)
{
    int nid = group->locals[localIdx].nid;
    uint64_t used = process->scanAttr.actcLen[nid];
    uint64_t reserve = group->locals[localIdx].localReservePages;
 ...
 uint64_t CalcGroupDeficit(ProcessAttr *process, const MigrationGroupAttr *group)
{
    uint64_t deficit = 0;
    for (int i = 0; i < group->localCount; i++) {
 ...
 uint64_t CalcGroupExcess(ProcessAttr *process, const MigrationGroupAttr *group)
{
    uint64_t excess = 0;
    for (int i = 0; i < group->localCount; i++) {
 ...
 bool IsGroupLocalSteady(ProcessAttr *process, const MigrationGroupAttr *group)
{
 ...
 uint64_t CalcRemoteUsed(const MigrationGroupAttr *group)
{
    uint64_t used = 0;
    for (int i = 0; i < group->targetCount; i++) {
 ...
 uint64_t CalcProcessTotalPages(const ProcessAttr *process)
{
    uint64_t total = 0;
 ...
 bool UpdateGroupSwapTotalPagesStable(ProcessAttr *process)
{
 ...
 void ResetGroupSwapCandidateRounds(GroupMigrationPolicy *policy)
{
    for (int i = 0; i < policy->groupCount; i++) {
 ...
 uint64_t CalcRemoteRemaining(const MigrationGroupAttr *group, int targetIdx)
{
 ...
 */
 int CollectTargetEntriesByNid(GroupMigrationPolicy *policy, int nid, GroupTargetEntry entries[], int maxEntries)
{
    int count = 0;
...
/* Calculate the total quota for entries sharing one remote NUMA. */
 uint64_t CalcTargetQuotaSum(GroupMigrationPolicy *policy, const GroupTargetEntry entries[], int entryCount)
{
    uint64_t quotaSum = 0;
 ...
 */
 void SyncSingleTargetUsedPages(ProcessAttr *process, GroupTargetEntry *entry, uint64_t residentPages)
{
 ...
 */
 void AssignSharedTargetUsedPages(ProcessAttr *process, const GroupTargetEntry entries[], int entryCount,
...
/* Validate shared-target quota accounting and publish the split usedPages. */
 void SyncSharedTargetUsedPages(ProcessAttr *process, const GroupTargetEntry entries[], int entryCount,
                                      uint64_t residentPages, uint64_t quotaSum)
{
    GroupMigrationPolicy *policy = &process->groupPolicy;
 ...
 * waterline.
 */
 void SyncGroupedTargetUsedPages(ProcessAttr *process)
{
    GroupMigrationPolicy *policy = &process->groupPolicy;
 ...
 bool IsSharedTarget(const GroupMigrationPolicy *policy, int groupIdx)
{
 ...
 uint64_t CalcGroupSwapBaseCap(uint64_t remoteUsed)
{
 ...
 void EnsureGroupedSwapParam(ProcessAttr *process, uint64_t localUsed, uint64_t remoteUsed)
{
 ...
 bool IsSwapBeneficial(const ProcessAttr *process, const GroupPageData *local, const GroupPageData *remote)
{
 ...
 int AppendMigPages(struct MigList *list, const GroupPageData *pages, uint64_t nr)
{
    if (nr == 0) {
        return 0;
    }
 ...
 int CollectLocalPages(ProcessAttr *process, const MigrationGroupAttr *group, GroupPageData **pages,
                             uint64_t *nrPages)
{
 ...
 uint64_t CollectPagesFromNode(ProcessAttr *process, int nid, uint64_t usedPages, uint64_t remoteOffset,
                                     GroupPageData *data, uint64_t startPos)
{
 ...
 int CollectRemotePages(ProcessAttr *process, const MigrationGroupAttr *group, const uint64_t remoteOffsets[],
                              GroupPageData **pages, uint64_t *nrPages)
{
 ...
 int BuildDemoteMigList(ProcessAttr *process, MigrationGroupAttr *group,
                              struct MigList mlist[MAX_NODES][MAX_NODES], uint64_t demoteNeed)
{
 ...
 int SelectPromoteLocal(ProcessAttr *process, const MigrationGroupAttr *group, uint64_t selectedPerLocal[])
{
    int selectedIdx = -1;
 ...
 int BuildPromoteMigList(ProcessAttr *process, MigrationGroupAttr *group, uint64_t remoteOffsets[],
                               struct MigList mlist[MAX_NODES][MAX_NODES], uint64_t promoteNeed)
{
 ...
 void GetPlannedPromoteToLocal(const MigrationGroupAttr *group, struct MigList mlist[MAX_NODES][MAX_NODES],
                                     uint64_t plannedIn[MAX_NODES])
{
 ...
 int SelectRebalanceTargetLocal(ProcessAttr *process, const MigrationGroupAttr *group,
                                      const uint64_t plannedIn[MAX_NODES], const uint64_t selectedPerLocal[])
{
 ...
 void FreeRebalanceBuckets(GroupPageData *pages[MAX_NODES][MAX_NODES])
{
 ...
 int BuildRebalanceMigListsFromPairs(const RebalancePair *pairs, uint64_t nrPairs,
                                           struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 int BuildLocalRebalanceMigList(ProcessAttr *process, MigrationGroupAttr *group,
                                      struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 void FreeSwapBuckets(GroupPageData *promotePages[MAX_NODES][MAX_NODES],
                            GroupPageData *demotePages[MAX_NODES][MAX_NODES])
{
 ...
 int AllocSwapBuckets(uint64_t promoteCnt[MAX_NODES][MAX_NODES], uint64_t demoteCnt[MAX_NODES][MAX_NODES],
                            ...
                            GroupPageData *demotePages[MAX_NODES][MAX_NODES])
{
 ...
 int BuildSwapMigListsFromPairs(const SwapPair *pairs, uint64_t nrPairs,
                                      struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 uint64_t SelectSwapPairs(ProcessAttr *process, const MigrationGroupAttr *group, GroupPageData *localPages,
                                ...
                                uint64_t swapCap, SwapPair *pairs)
{
 ...
 int BuildSwapMigList(ProcessAttr *process, MigrationGroupAttr *group, struct MigList mlist[MAX_NODES][MAX_NODES],
                            uint64_t localUsed, uint64_t remoteUsed, uint64_t *nrBuilt)
{
 ...
 int BuildPromoteGroups(ProcessAttr *process, PromoteGroup *groups, int *groupCnt)
{
    *groupCnt = 0;
 ...
 int RunPromoteStage(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 int RunDemoteStage(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES])
{
    for (int i = 0; i < process->groupPolicy.groupCount; i++) {
        MigrationGroupAttr *group = &process->groupPolicy.groups[i];
 ...
 int RunLocalRebalanceStage(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES])
{
    for (int i = 0; i < process->groupPolicy.groupCount; i++) {
        MigrationGroupAttr *group = &process->groupPolicy.groups[i];
        uint64_t deficit = CalcGroupDeficit(process, group);
 ...
 int RunSwapStage(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES])
{
