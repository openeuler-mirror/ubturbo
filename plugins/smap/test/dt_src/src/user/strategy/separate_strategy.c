...
typedef struct NumaInfo {
 ...
 int InitSeparateParam(ProcessAttr *process)
{
 ...
 bool ShouldMigrate(ProcessAttr *process)
{
 ...
 void FreeMlist(struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 actc_t FindKthFreqAsc(const uint32_t *buckets, uint64_t k)
{
    if (k == 0) {
        return 0;
    }
 ...
 actc_t FindKthFreqDesc(const uint32_t *buckets, uint64_t k)
{
    if (k == 0) {
        return 0;
    }
 ...
 uint64_t BuildFreqBucketsForNode(ProcessAttr *process, int nid, uint64_t offset, uint32_t *buckets)
{
    uint64_t count = 0;
 ...
 uint64_t CalcFreqWeightForVm(ProcessAttr *process, int localNid, int remoteNid, const uint32_t *l2Buckets,
                                    uint64_t l2ActcLen)
{
    uint64_t freqWt;
 ...
 uint64_t CalLowMigrateNumByBuckets(uint64_t migrateNum, uint64_t freqWt, uint32_t slowThred,
                                          ...
                                          uint64_t l2Count)
{
 ...
 uint64_t CalcSwapNumForVm(ProcessAttr *process, int localNid, int remoteNid, const uint64_t numaOffset[],
                                 const uint64_t numaFreePage[])
{
 ...
 uint64_t CalcSwapNum(ProcessAttr *process, int localNid, int remoteNid, const uint64_t numaOffset[],
                            const uint64_t numaFreePage[])
{
                   ...
                   uint32_t *takeAtThreshold)
{
 ...
 size_t CollectPagesVm(const SelectionMode mode, uint64_t actcLen, ActcData *currentData,
                             ...
                             uint32_t *selectedBuckets)
{
 ...
 size_t CollectPages4K(const SelectionMode mode, uint64_t actcLen, ActcData *currentData,
                             struct MigList *currMlist, uint64_t nrMig, int thresholdFreq, uint32_t takeAtThreshold,
                             uint32_t *selectedBuckets)
{
    int nrLocalNuma = GetNrLocalNuma();
 ...
 int BuildSelectKMlistAddr(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
                                 const uint64_t numaOffset[MAX_NODES], int from, int to, SelectionMode mode)
{
 ...
 int BuildPairMlist(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
...
int PairMigrationStrategy(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 int SeparateStrategyInner(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
                                 uint64_t numaOffset[MAX_NODES], struct NumaInfo info, uint64_t swapNum)
{
 ...
 uint64_t CalcAdditionalPage(ProcessAttr *process, int localNid, int remoteNid, uint64_t aimNum,
                                   const uint64_t numaOffest[], const uint64_t numaFreePage[])
{
 ...
 void UpdateSwapNum(ProcessAttr *process, uint64_t swapNum[LOCAL_NUMA_BITS][MAX_NODES], int localNumaNum,
...
int SeparateStrategy(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 int FreqAscFunc(const void *actc1, const void *actc2)
{
 ...
 int AddrAscFunc(const void *a, const void *b)
{
 ...
 int FreqDescFunc(const void *actc1, const void *actc2)
{
    LevelActcData *a1 = (LevelActcData *)actc1;
    LevelActcData *a2 = (LevelActcData *)actc2;
 ...
 uint64_t CalMultiNumaVmLowMigrateNum(uint64_t migrateNum, uint64_t freqWt, uint32_t slowThred,
                                            LevelActcData *levelActcData[NR_LEVEL])
{
 ...
 uint64_t CalcMultiNumaVmSwapNumByFreq(ProcessAttr *process, LevelActcData *levelActcData[NR_LEVEL],
                                             uint64_t levelActcLen[NR_LEVEL])
{
 ...
 int GroupMigPagesByNode(LevelActcData *levelActcData[NR_LEVEL], uint64_t swapNum, uint64_t nrMig[MAX_NODES],
                               uint64_t *migAddrArray[MAX_NODES])
{
 ...
 int BuildMigListForDirection(MigrateDirection dir, uint64_t *migAddrArray[MAX_NODES], uint64_t nrMig[MAX_NODES],
                                    uint64_t destFreeList[MAX_NODES], struct MigList mlist[MAX_NODES][MAX_NODES])
{
 ...
 int BuildDemoteMultiNumaMigLists(uint64_t *migAddrArray[MAX_NODES], uint64_t nrMig[MAX_NODES],
                                        ...
                                        RemoteMigInfo remoteMigInfo[REMOTE_NUMA_NUM])
{
 ...
 void CountMigrationPerNode(uint64_t nrMig[MAX_NODES], LevelActcData *levelActcData[NR_LEVEL], uint64_t swapNum,
                                  int nrLocalNuma)
{
    for (uint64_t i = 0; i < swapNum; i++) {
        int l1node = levelActcData[L1][i].node;
 ...
 void FreeMultiNumaVmData(LevelActcData *levelActcData[NR_LEVEL], uint64_t *migAddrArray[MAX_NODES])
{
 ...
 int BuildLevelActcData(ProcessAttr *process, LevelActcData *levelActcData, uint64_t nrPages,
                              uint64_t *levelActcLen, int level)
{
 ...
 int PromoteMultiNumaVmStrategy(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
                                      RemoteMigInfo remoteMigInfo[REMOTE_NUMA_NUM], int nrLocalNuma)
{
 ...
 void FreeDemoteMultiNumaVmData(LevelActcData *l1ActcData, uint64_t *migAddrArray[MAX_NODES])
{
    if (l1ActcData) {
        free(l1ActcData);
    }
 ...
 int DemoteMultiNumaVmStrategy(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
...
cleanup:
 ...
 int BuildSwapMigLists(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
                             uint64_t nrMig[MAX_NODES], uint64_t *migAddrArray[MAX_NODES], int nrLocalNuma)
{
 ...
 int SwapMultiNumaVmStrategy(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES],
                                   uint64_t nrPages[NR_LEVEL], int nrLocalNuma)
{
 ...
 void CalculateMigInfo(ProcessAttr *process, RemoteMigInfo remoteMigInfo[REMOTE_NUMA_NUM],
