...
unsigned int major;
 dev_t ucache_dev;
 struct cdev ucache_cdev;
 struct class_stub *ucache_dev_class;
 struct folio *folios[NR_TO_SCAN];
 ...
 int ucache_query_migrate_success(uintptr_t arg)
{
 ...
 int dev_ucache_migrate_folios(uintptr_t arg)
{
	int ret = 0;
 ...
 long ucache_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
