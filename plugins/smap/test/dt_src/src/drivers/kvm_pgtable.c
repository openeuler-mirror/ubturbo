...
#define KVM_PTE_LEAF_ATTR_HI_S2_XN BIT(54)
 ...
 u32 kvm_pgtable_idx(struct kvm_pgtable_walk_data *data, u32 level)
{
 ...
 bool kvm_pte_table(kvm_pte_t pte, u32 level)
{
 ...
 int kvm_pgtable_visitor_cb(struct kvm_pgtable_walk_data *data,
				  ...
				  enum kvm_pgtable_walk_flags visit)
{
 ...
 bool kvm_pgtable_walk_continue(const struct kvm_pgtable_walker *walker,
				      int r)
{
	/*
 ...
 kvm_pte_t *kvm_pte_follow(kvm_pte_t pte,
				 struct kvm_pgtable_mm_ops *mm_ops)
{
	return mm_ops->phys_to_virt(kvm_pte_to_phys(pte));
}

 int __kvm_pgtable_walk(struct kvm_pgtable_walk_data *data,
			      struct kvm_pgtable_mm_ops *mm_ops,
			      kvm_pteref_t pgtable, u32 level);

  int __kvm_pgtable_visit(struct kvm_pgtable_walk_data *data,
				      struct kvm_pgtable_mm_ops *mm_ops,
			      ...
			      kvm_pteref_t pgtable, u32 level)
{
 ...
 u32 kvm_pgd_page_idx(struct kvm_pgtable *pgt, u64 addr)
{
 ...
 int _kvm_pgtable_walk(struct kvm_pgtable *pgt,
		     ...
		     struct kvm_pgtable_walker *walker)
{
 ...
 bool stage2_try_set_pte(const struct kvm_pgtable_visit_ctx *ctx,
			       kvm_pte_t new)
{
...
struct smap_stage2_test_clear_young_data {
  ...
  bool stage2_pte_executable(kvm_pte_t pte)
{
				    ...
				    enum kvm_pgtable_walk_flags visit)
{
 ...
 int smap_stage2_test_clear_young(struct kvm_pgtable *pgt, u64 addr,
...
bool smap_kvm_pgtable_stage2_is_young(struct kvm_pgtable *pgt, u64 addr)
{
 ...
 int smap_stage2_range_walker(const struct kvm_pgtable_visit_ctx *ctx,
				    enum kvm_pgtable_walk_flags visit)
{
	kvm_pte_t pte = ctx->old;
	kvm_pte_t new = ctx->old & ~KVM_PTE_LEAF_ATTR_LO_S2_AF;
