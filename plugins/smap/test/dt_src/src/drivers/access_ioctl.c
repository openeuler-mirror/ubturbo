...
#define MAX_2M_PROCESSES_CNT 100
 ...
 dev_t ioctl_access_dev;
 struct class_stub *access_class;
 struct cdev access_cdev;
 struct device *access_device;
 struct user_info ubturbo_ui = { 0 };
kuid_t procfs_kuid;
kgid_t procfs_kgid;
struct proc_dir_entry *smap_procfs_root = NULL;

 char *smap_bitmap_buf = NULL;
 size_t smap_buf_len = 0;

 int check_msg_validity(struct access_add_pid_msg *msg)
{
	if (!msg) {
 ...
 int add_payload(int len, struct access_add_pid_payload *payload,
		       int page_size)
{
	int ret;
 ...
 long ioctl_add_pid(void __user *argp)
{
	int ret = 0, i = 0;
...
out_free_payload:
 ...
 long ioctl_remove_pid(void __user *argp)
{
	int i;
 ...
 long ioctl_remove_all_pid(void __user *argp)
{
 ...
 void remove_procfs_root(void)
{
	proc_remove(smap_procfs_root);
	smap_procfs_root = NULL;
}

 int create_procfs_root(struct user_info *ui)
{
  ...
  bool is_user_unchanged(struct user_info *ui)
{
	return ui && ui->uid == ubturbo_ui.uid && ui->gid == ubturbo_ui.gid;
}

 long ioctl_create_smap_procfs(void __user *argp)
{
	int ret;
 ...
 size_t calc_bitmap_len(void)
{
 ...
 long ioctl_walk_pagemap(void __user *argp)
{
	size_t buf_len;
  ...
  void write_bitmap_pid(char **buffer, struct access_pid *ap)
{
  ...
  void write_bitmap_nrpage(char **buffer, struct access_pid *ap)
{
	int i;
	if (unlikely(!buffer || !(*buffer) || !ap)) {
 ...
 void write_bitmap_buffer(char **buffer)
{
 ...
 ssize_t read_bitmap(char __user *buf, size_t cnt, loff_t *loff,
			   bool *completed)
{
	char *tmp_buf;
	ssize_t len;
...
free_buf:
 ...
 int smap_access_open(struct inode *inode, struct file *file)
{
	return 0;
}

 int smap_access_release(struct inode *inode, struct file *file)
{
	return 0;
}

 void update_tracking_data(u16 *tracking_data,
				 ...
				 struct tracking_info_payload *payload_info)
{
 ...
 long ioctl_get_tracking(void __user *argp)
{
	int ret = 0;
...
out_free:
 ...
 long ioctl_get_nr_local_numa(void __user *argp)
{
 ...
 long ioctl_set_scan_cpu(void __user *argp)
{
 ...
 long smap_access_ioctl(struct file *file, unsigned int cmd,
			      unsigned long arg)
{
 ...
 ssize_t smap_access_read(struct file *file, char __user *buf, size_t cnt,
				loff_t *loff)
{
	ssize_t ret;
 ...
 struct file_operations smap_access_fops = {
	.owner = THIS_MODULE,
...
int access_dev_init(void)
{
	...
	if (IS_ERR(access_class)) {
		pr_err("unable to create the access class_stub\n");
