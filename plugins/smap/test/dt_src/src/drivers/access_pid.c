...
void destroy_access_pid(struct access_pid *elem)
{
 ...
 void destroy_access_ham_pid(struct ham_tracking_info *elem)
{
	if (!elem) {
		return;
	}
 ...
 void destroy_access_statistic_pid(struct statistics_tracking_info *elem)
{
	u64 i;
	if (!elem) {
		return;
 ...
 void add_kvm_seg(struct vm_mapping_info *info,
			...
			u64 gfn_end)
{
 ...
 int scan_kvm_gfn(struct kvm *kvm, struct vm_mapping_info *info)
{
  ...
  int pre_scan_kvm_gfn(struct kvm *kvm)
{
  ...
  void post_scan_kvm_gfn(struct kvm *kvm, int idx)
{
...
int init_vm_mapping(struct vm_mapping_info *info)
{
 ...
 int init_vm_mapping_info(pid_t pid, struct vm_mapping_info *info)
{
	int srcu_idx, ret;
	struct file *filp;
	struct kvm *kvm;
 ...
 int mem_freq_open(struct inode *inode, struct file *file)
{
	file->private_data = pde_data(inode);
	return 0;
}

 int mem_freq_release(struct inode *inode, struct file *file)
{
	return 0;
}
 ...
 */
 void fill_actc_data_by_bitmap(struct access_pid *ap, int nid,
 ...
 */
 void fill_pid_actc_data(struct access_pid *ap, struct actc_data *actc,
			       u32 actc_len[SMAP_MAX_NUMNODES])
{
  ...
  size_t calc_process_page_number(struct access_pid *ap)
{
	int i;
	size_t ret_len = 0;
	for (i = 0; i < SMAP_MAX_NUMNODES; i++) {
 ...
 ssize_t mem_freq_read(struct file *file, char __user *buf, size_t cnt,
			     loff_t *ppos)
{
...
out:
 ...
 const struct proc_ops ap_mem_freq_fops = {
	.proc_open = mem_freq_open,
	.proc_release = mem_freq_release,
	.proc_read = mem_freq_read,
};

 int create_procfs_freq(struct access_pid *ap)
{
 ...
 bool pid_procfs_exists(pid_t pid)
{
	int ret;
	struct path p;
 ...
 int create_procfs(struct access_pid *ap)
{
	int ret;
...
void print_access_statistic_pid_list(void)
{
 ...
 int init_ham_pid_memory(struct ham_tracking_info *info,
			       enum node_level level)
{
  ...
  void clean_ham_tracking_info(struct ham_tracking_info *tmp)
{
	if (tmp->paddr[L1]) {
		vfree(tmp->paddr[L1]);
	}
 ...
 int init_ham_pid_len(struct ham_tracking_info *tmp)
{
	struct access_tracking_dev *adev;
  ...
  int find_first_local_numa(unsigned long *nodes)
{
	unsigned long pos;
	unsigned long size;
 ...
 int find_first_remote_numa(unsigned long *nodes)
{
	unsigned long pos;
	int nid;
	unsigned long size;
 ...
 int init_access_ham_pid(struct access_add_pid_payload *payload)
{
...
int access_add_ham_pid(int len, struct access_add_pid_payload *payload)
{
 ...
 int init_statistic_window(u8 ***sliding_windows, u32 duration,
				 ...
				 u64 *windows_num)
{
 ...
 int init_access_statistic_pid_2m(struct access_add_pid_payload *payload)
{
 ...
 int init_access_statistic_pid_4k(struct access_add_pid_payload *payload)
{
	struct statistics_tracking_info *tmp, *ap;
	int l1_node, l2_node, ret;
 ...
 int init_access_statistic_pid(struct access_add_pid_payload *payload,
				     int page_size)
{
			     ...
			     int page_size)
{
 ...
 void move_to_ap_data_list(struct list_head *tmp_head)
{
...
void access_remove_all_pid(void)
{
  ...
  void free_ap_bm(struct access_pid *ap)
{
	int i;
	if (!ap) {
		return;
 ...
 void free_ap_bm_white_list(struct access_pid *ap)
{
	int i;
	if (!ap) {
		return;
...
struct absolute_pos {
 ...
 int calc_absolute_pos(unsigned long *paddr_bm, size_t bm_len,
...
#define INITIAL_POS_INDEX (-1)
 ...
 int check_parameters_and_state(u64 len, u64 *addr)
{
	if (len == 0 || !addr) {
 ...
 void convert_single_address(struct access_pid *ap, int nid,
