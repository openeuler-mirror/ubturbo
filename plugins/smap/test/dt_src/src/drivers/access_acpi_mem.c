...
int drivers_nr_local_numa = 0;
struct mem_info drivers_acpi_mem = {
	.len = 0,
	.mem = LIST_HEAD_INIT(drivers_acpi_mem.mem),
};

bool drivers_is_paddr_local(u64 pa)
{
	struct acpi_mem_segment *mem;
	list_for_each_entry(mem, &drivers_acpi_mem.mem, segment) {
 ...
 void drivers_update_nr_local_numa(int node)
{
	if (node >= drivers_nr_local_numa) {
		drivers_nr_local_numa = node + 1;
		pr_info("local NUMA nodes amount: %u (from node %d)\n",
			drivers_nr_local_numa, node);
	}
}

 int drivers_acpi_table_build_mem(struct acpi_subtable_header *header)
{
	int node;
	u32 hotpluggable;
	...
	drivers_update_nr_local_numa(mem->node);

	/* Add to list and ensure the ascending order of drivers_acpi_mem.mem */
	if (list_empty(&drivers_acpi_mem.mem)) {
		list_add_tail(&mem->segment, &drivers_acpi_mem.mem);
	} else {
		list_for_each_entry_reverse(tmp, &drivers_acpi_mem.mem, segment) {
			if (tmp->start < mem->start)
				break;
		}
		if (list_entry_is_head(tmp, &drivers_acpi_mem.mem, segment))
			list_add(&mem->segment, &drivers_acpi_mem.mem);
		else
			list_add(&mem->segment, &tmp->segment);
	}
	drivers_acpi_mem.len++;

	return 0;
}

 int drivers_acpi_parse_memory_affinity(union acpi_subtable_headers *header,
				      const unsigned long end)
{
	struct acpi_srat_mem_affinity *memory_affinity;

	memory_affinity = (struct acpi_srat_mem_affinity *)header;
	return drivers_acpi_table_build_mem(&header->common);
}

 int drivers_acpi_parse_gicc_affinity(union acpi_subtable_headers *header,
				    const unsigned long end)
{
	...
	drivers_update_nr_local_numa(node);

	return 0;
}

 void drivers_merge_acpi_mem_segments(void)
{
	...
	cur = list_first_entry(&drivers_acpi_mem.mem, struct acpi_mem_segment, segment);
	while (cur) {
		next = list_next_entry(cur, segment);
		if (list_entry_is_head(next, &drivers_acpi_mem.mem, segment))
			break;
...
int drivers_init_acpi_mem(void)
{
	int count;
	acpi_status status;
	...
	struct acpi_subtable_proc proc[] = {
		{
			...
			.handler = drivers_acpi_parse_memory_affinity,
		},
		{
			.id = ACPI_SRAT_TYPE_GICC_AFFINITY,
			.handler = drivers_acpi_parse_gicc_affinity,
		},
	};
	...
	pr_info("drivers_init_acpi_mem: drivers_nr_local_numa = %d\n", drivers_nr_local_numa);
	drivers_merge_acpi_mem_segments();

	return 0;
}

void drivers_reset_acpi_mem(void)
{
	struct acpi_mem_segment *mem, *tmp;
	list_for_each_entry_safe(mem, tmp, &drivers_acpi_mem.mem, segment) {
...
u64 drivers_get_node_actc_len(int node_id, int page_size)
{
	u64 page_cnt = 0;
	struct acpi_mem_segment *mem;
