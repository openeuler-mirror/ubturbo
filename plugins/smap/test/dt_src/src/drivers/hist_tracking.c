...
extern struct list_head access_dev;
  ...
  void drivers_reset_actc_data(struct access_tracking_dev *hdev)
{
 ...
 int hist_tracking_disable(struct device *ldev)
{
 ...
 void drivers_actc_buffer_deinit(struct access_tracking_dev *hdev)
{
	hdev->page_count = 0;
	if (hdev->access_bit_actc_data) {
  ...
  int hist_get_page_size(struct access_tracking_dev *hdev)
{
 ...
 u64 drivers_calc_access_len(struct access_tracking_dev *hdev)
{
 ...
 void hist_dev_pgsize_update(u8 page_size_mode)
{
	u32 pgsize = page_size_mode == PAGE_MODE_2M ? SIZE_2M : SIZE_4K;
	hist_update_pgsize(pgsize);
}

 int drivers_actc_buffer_reinit(struct access_tracking_dev *hdev)
{
	u64 page_count;
	page_count = drivers_calc_access_len(hdev);
	if (!page_count) {
		pr_debug("no page found on node: %d\n", hdev->node);
	}
	if (hdev->page_count == page_count) {
		drivers_reset_actc_data(hdev);
		return 0;
	}
	drivers_actc_buffer_deinit(hdev);
	if (page_count) {
 ...
 void hist_tracking_enable(struct device *ldev)
{
	struct access_tracking_dev *hdev;
	int ret;
	...
	ret = drivers_actc_buffer_reinit(hdev);
	if (ret) {
 ...
 int hist_tracking_set_page_size(struct device *ldev, u8 pgsize)
{
	int ret;
	struct access_tracking_dev *hdev;
	hdev = to_access_tracking_dev(ldev);
  ...
  bool is_numa_flux_updated(struct ub_flux_mb_statistic *stc,
					int numa_id)
{
	int i;
 ...
 int hist_tracking_ub_watch(struct device *ldev, void *result)
{
 ...
 int hist_tracking_ub_watch_config(struct device *ldev, u32 duration_ms)
{
	return ub_watch_config(duration_ms);
}

 struct tracking_operations g_hist_tracking_ops = {
 ...
 int drivers_actc_buffer_init(struct access_tracking_dev *hdev)
{
	hdev->page_count = drivers_calc_access_len(hdev);
 ...
 void hist_tracking_deinit(void)
{
 ...
 int hist_tracking_init(void)
{
	...
	for (node = (unsigned int)nr_local_numa; node < SMAP_MAX_NUMNODES;
		...
		ret = drivers_actc_buffer_init(hdev);
		if (ret) {
