...
#define DELAY_BUFFER_MS 8
 ...
 void drivers_work_func(struct work_struct *work);
...
void cancel_ap_scan_work(struct access_pid *ap)
{
 ...
 void set_scan_cpus_work_fn(struct work_struct *work)
{
...
void submit_one_work(struct access_pid *ap)
{
	...
	INIT_DELAYED_WORK(&ap->scan_work, drivers_work_func);
	queue_delayed_work(adev_head->scanq, &ap->scan_work,
			   msecs_to_jiffies(ap->scan_time));
}

 void submit_scan_works(struct access_tracking_dev *adev)
{
 ...
 int create_scan_workqueue(void)
{
 ...
 void destroy_scan_workqueue(void)
{
	struct access_pid *ap;
	struct access_tracking_dev *adev = get_first_access_dev();
	down_read(&ap_data.lock);
  ...
  void drivers_init_actc_data(struct access_tracking_dev *adev)
{
 ...
 void access_print_acpi_mem(void)
{
#ifdef DEBUG
 ...
 u64 calc_access_len_v2(struct access_tracking_dev *adev)
{
 ...
 void actc_buffer_deinit(struct access_tracking_dev *adev)
{
	adev->page_count = 0;
	if (adev->access_bit_actc_data) {
 ...
 int actc_buffer_reinit(struct access_tracking_dev *adev)
{
	u64 page_count;
 ...
 void access_tracking_enable(struct device *ldev)
{
 ...
 int access_tracking_disable(struct device *ldev)
{
	struct access_tracking_dev *adev = to_accessbit_dev(ldev);
	struct access_pid *ap;
	 ...
	 *
	 * complete(&ap->work_done) 在 drivers_work_func 释放 ap_data.lock 读锁之后才
 ...
 int access_tracking_mode_set(struct device *ldev, u8 mode)
{
	struct access_tracking_dev *adev = to_accessbit_dev(ldev);
 ...
 int access_tracking_set_page_size(struct device *ldev,
					 u8 page_size_index)
{
	int ret;
	struct access_tracking_dev *adev = to_accessbit_dev(ldev);
 ...
 struct tracking_operations access_tracking_ops = {
...
int calc_access_len(struct access_tracking_dev *adev)
{
 ...
 int actc_buffer_init(struct access_tracking_dev *adev)
{
...
put_dev:
 ...
 void adev_buffer_down_read(void)
{
	struct access_tracking_dev *adev;
 ...
 void adev_buffer_up_read(void)
{
 ...
 void handle_statistic_scan(struct access_pid *ap, ktime_t start_time,
 ...
 void release_adev(void)
{
 ...
 int __init access_tracking_init(void)
{
	int ret = 0;
...
err_remote_ram:
 ...
 void __exit access_tracking_exit(void)
{
	access_ioctl_exit();
