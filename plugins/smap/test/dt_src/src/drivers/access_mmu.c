...
#define MAPPING_PRIOR_BIT 6
  ...
  pagemap_entry_t make_pme(u64 frame, u64 flags)
{
	return (pagemap_entry_t){ .pme = (frame & PM_PFRAME_MASK) | flags };
}

 int mock_calc_paddr_acidx(u64 paddr, int *nid, u64 *index)
{
	int page_size = is_access_hugepage() ? g_pagesize_huge : PAGE_SIZE;
	if (drivers_is_paddr_local(paddr))
		return calc_paddr_acidx_acpi(paddr, nid, index, page_size);
	return drivers_calc_paddr_acidx_iomem(paddr, nid, index, page_size);
}
...
int add_to_bm_page(u64 paddr, struct access_pid *ap)
{
	int nid, nid_pos, ret;
	u64 acidx;
	ret = mock_calc_paddr_acidx(paddr, &nid, &acidx);
	if (ret)
		return ret;
...
int add_to_bm_page_fast(u64 paddr, int nid, u64 acidx, struct access_pid *ap)
{
 ...
 int calc_vaddr_acidx(u64 vaddr, struct vm_mapping_info *info, u64 *acidx)
{
	int i;
	*acidx = 0;
 ...
 void set_pa_prior(struct access_pid *ap, u64 vaddr)
{
	u8 prior;
	int prior_bits;
	u64 va_idx;
...
int add_to_bm_hugepage(u64 vaddr, u64 paddr, struct access_pid *ap)
{
 ...
 bool is_paddr_belong_remote_node(u64 pa, int nid)
{
 ...
 bool is_page_ready_for_migrate(struct page *page)
{
 ...
 int add_to_bm(unsigned long vaddr, pagemap_entry_t *pme,
...
spinlock_t *__pmd_trans_huge_lock(pmd_t *pmd, struct vm_area_struct *vma)
{
 ...
 int pagemap_pte_range(pte_t *pte, unsigned long addr, unsigned long next,
			     struct mm_walk *walk)
{
	struct vm_area_struct *vma = walk->vma;
	struct pagemapread *pm = walk->private_data;
	pagemap_entry_t pme;
 ...
 int pagemap_hugetlb_range(pte_t *ptep, unsigned long hmask,
				 ...
				 struct mm_walk *walk)
{
 ...
 const struct mm_walk_ops pagemap_mm_ops = {
	.pte_entry = pagemap_pte_range,
};

 const struct mm_walk_ops pagemap_vm_ops = {
	.hugetlb_entry = pagemap_hugetlb_range,
};

 void pos_to_addr(struct mm_struct *mm, unsigned long pos,
			unsigned long *vaddr)
{
...
void walk_pid_pagemap(struct pagemapread *pm)
{
	...
	/* 获取mm_struct */
	mm = mock_get_mm_by_pid(pm->mig_info.pid);
	if (IS_ERR(mm))
		return;
