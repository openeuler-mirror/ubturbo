...
struct obmm_dev_info obmm_dev = {
 ...
 void free_remote_ram(struct list_head *head)
{
 ...
 void copy_remote_ram(struct list_head *dst, struct list_head *src)
{
	struct ram_segment *seg, *tmp;
 ...
 void merge_ram_segments(struct list_head *head)
{
 ...
 int update_resource(struct resource *r, void *arg)
{
 ...
 int walk_system_ram_remote_range(struct list_head *head)
{
	if (!head)
		return -EINVAL;
 ...
 int init_obmm_dev(void)
{
 ...
 void update_obmm_dev(u64 memid)
{
	struct memid_range *mr, *tmp;
	u64 std_seq;
 ...
 int extract_hex_content(const char *file_path, u64 *content)
{
	struct file *filp;
 ...
 void clean_obmm_dev(void)
{
  ...
  bool is_import_shmdev(const char *name)
{
...
struct read_obmm_callback {
 ...
 bool fill_obmmdev(struct dir_context *ctx, const char *name, int namelen,
