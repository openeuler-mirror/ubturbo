...
#define MAX_MIGRATE_PID_NUMA_RETRY_TIME 100
 ...
 dev_t mig_dev = 0;
 struct class_stub *mig_dev_class;
 struct cdev smu_mig_cdev;
 struct device *smu_mig_device;

 void free_migrate_list_addr(int len, struct mig_list *mlist)
{
	int i;
 ...
 void free_migrate_list(struct mig_list **mlist)
{
	if (*mlist) {
		vfree(*mlist);
		*mlist = NULL;
 ...
 int create_migrate_list(struct migrate_msg *msg, struct mig_list **mlist)
{
 ...
 int init_migrate_list_addr(int len, struct mig_list *mlist)
{
	int ret;
	int i, j;
	struct mig_list *ml;
...
out:
 ...
 int convert_migrate_list(int len, struct mig_list *mlist)
{
	int i, ret;
 ...
 int build_migrate_list(struct migrate_msg *msg, struct mig_list **mlist)
{
	int ret;
 ...
 bool is_migrate_msg_valid(struct migrate_msg *msg)
{
 ...
 int __ioctl_migrate(void __user *argp)
{
 ...
 int __ioctl_migrate_numa(void __user *argp)
{
	int i, ret;
 ...
 int check_mig_msg(struct mig_payload *payloads, int len)
{
	for (int i = 0; i < len; i++) {
 ...
 void init_pm_info(struct pagemapread *pm, struct mig_payload *payload)
{
	int page_size = smap_pgtype == HUGE_PAGE ? g_pagesize_huge : PAGE_SIZE;
 ...
 void walkpage_and_migrate(struct mig_payload *payloads, int len,
				 int *mig_res)
{
	int i;
	int cnt;
 ...
 int copy_to_user_mig_res(struct migrate_pid_remote_numa_msg *msg,
				...
				int *mig_res)
{
 ...
 int __ioctl_migrate_pid_remote_numa(void __user *argp)
{
	int ret = 0;
	int *mig_res = NULL;
 ...
 int __ioctl_set_pagetype(void __user *argp)
{
	uint32_t pageType;
 ...
 int __ioctl_set_ub_dma_avail(void __user *argp)
{
 ...
 long smu_mig_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg)
{
...
out:
 ...
 int smu_mig_open(struct inode *inode, struct file *file);
 int smu_mig_ioctl_release(struct inode *inode, struct file *file);
 long smu_mig_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg);

 struct file_operations smu_mig_fops = {
	.owner = THIS_MODULE,
	.open = smu_mig_open,
...
int init_mig_dev(void)
{
	...
	if (IS_ERR(mig_dev_class)) {
		pr_err("unable to create SMAP migrate class_stub\n");
