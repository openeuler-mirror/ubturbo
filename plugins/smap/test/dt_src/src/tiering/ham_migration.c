...
typedef struct folio *get_migration_folio_t(struct ham_page_map *hpm);
 ...
 struct cdev g_ham_cdev;
 dev_t g_ham_dev;
 struct class_stub *g_ham_class;
 struct device *g_ham_device;

 struct workqueue_struct *ham_wq;
 struct delayed_work ham_work;

 bool global_cache_mnt = false;
...
struct folio *ham_alloc_huge_page_node(int nid)
{
 ...
 int config_system_huge_page(unsigned long huge_page_number,
				   unsigned int node_id, struct hstate *h)
{
 ...
 int compare_page_list(const void *a, const void *b)
{
 ...
 int compare_page_freq(const void *a, const void *b)
{
 ...
 int init_numa_page_map(struct ham_migrate_task *mig_task,
...
out_err:
 ...
 struct ham_migrate_task *init_migrate_task(struct migration_param *arg,
						  struct hstate *h)
{
 ...
 void reclaim_task_fn(struct work_struct *work)
{
 ...
 int init_global_task_list(void)
{
	init_task_list();
 ...
 void update_mems_allowed_with_remote_nodes(struct migration_param *param)
{
	unsigned int i;
 ...
 struct folio *alloc_folio_with_retry(int nid,
					    struct ham_migrate_task *mig_task,
					    struct migration_param *param)
{
 ...
 int create_numa_map(struct ham_migrate_task *mig_task,
			   struct migration_param *param)
{
 ...
 int fill_folios_hugetlb(pte_t *pte, unsigned long hmask,
			       unsigned long addr, unsigned long end,
			       struct mm_walk *walk)
{
	struct ram_block_map *ram_map = walk->private_data;
	struct folio *folio;
	pte_t entry;
	unsigned long index;
 ...
 */
 int fill_task_pages(struct ham_migrate_task *mig_task)
{
 ...
 void sort_hpm_list(struct list_head *head, unsigned int nr_hpm)
{
 ...
 * of the ret_hpm_list.
 */
 unsigned int queue_qualified_pages(struct ham_migrate_task *mig_task,
					  bool migrated,
 ...
 */
 int handle_ham_migration(struct list_head *hpm_list,
				...
				free_folio_t put_new_folio)
{
 ...
 struct folio *get_folio_migrate_out(struct ham_page_map *hpm)
{
	return hpm_test_migrate(hpm) ? NULL : hpm->src_folio;
}

 struct folio *get_folio_migrate_back(struct ham_page_map *hpm)
{
...
void put_new_folio_rollback(struct folio *folio, unsigned long private)
{
 ...
 int ham_cache_clear(pid_t pid, struct ham_migrate_task *mig_task)
{
	unsigned int i;
	int ret = 0;
 ...
 int src_suspend_pgtable_maintain(struct ham_migrate_task *mig_task)
{
	int ret;
	unsigned int i;
	ktime_t t1, t2, t3;
 ...
 int src_pgtable_maintain_rollback(struct ham_migrate_task *mig_task)
{
	unsigned int i;
	int ret = 0;
	struct ram_block_map *ram_map;
 ...
 int dst_resume_pgtable_maintain(struct ham_migrate_task *mig_task)
{
	int ret;
	unsigned int i;
	struct ram_block_map *ram_map;
 ...
 void fill_freqs_to_rbm(struct ram_block_map *rbm,
			      ...
			      int freq_info_num)
{
 ...
 int get_folios_freqs(struct ham_migrate_task *mig_task)
{
	unsigned int i;
	int ret;
	struct ram_block_map *map;
  ...
  int ham_modify_pgtable(struct ham_migrate_task *mig_task,
				     bool cacheable)
{
  ...
  bool pid_legally(pid_t pid)
{
	return current->tgid == pid;
}

 int check_rmt_numa_info(struct ram_block_info *rbi)
{
 ...
 int check_migration_param(struct migration_param param,
				 struct hstate **h)
{
	struct mm_struct *mm;
...
exit_with_mmput:
 ...
 long ioctl_start_migration(unsigned long arg)
{
	struct ham_migrate_task *mig_task;
	struct migration_param param;
...
out_err:
 ...
 long ioctl_migration_pages(unsigned long arg)
{
	struct ham_migrate_task *mig_task;
	LIST_HEAD(hpm_list);
	unsigned int nr_hpm;
...
out_put:
 ...
 long ioctl_rollback_pages(unsigned long arg)
{
	struct ham_migrate_task *mig_task;
	LIST_HEAD(hpm_list);
	unsigned int nr_hpm;
...
out:
 ...
 long ioctl_stop_migration(unsigned long arg)
{
	struct ham_migrate_task *mig_task;
	pid_t pid;
	int ret;
...
out:
 ...
 long ioctl_modify_pagetable(unsigned long arg)
{
...
out:
 ...
 long ioctl_cache_clear(unsigned long arg)
{
	pid_t pid;
	int ret;
	ktime_t t1, t2;
...
out:
 ...
 int ham_open(struct inode *inode, struct file *file)
{
	return 0;
}

 int ham_release(struct inode *inode, struct file *file)
{
	return 0;
}

 ssize_t ham_read(struct file *file, char __user *buf, size_t size,
			loff_t *ppos)
{
	return 0;
}

 ssize_t ham_write(struct file *file, const char __user *buf,
			 size_t count, loff_t *ppos)
{
	return 0;
}

 long ham_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
 ...
 const struct file_operations g_ham_fops = {
	.owner = THIS_MODULE,
	.open = ham_open,
	.release = ham_release,
	.read = ham_read,
 ...
 ssize_t global_cache_mnt_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", global_cache_mnt);
}

 ssize_t global_cache_mnt_store(struct device *dev,
				      ...
				      const char *buf, size_t count)
{
 ...
 DEVICE_ATTR(global_cache_mnt, 0664, global_cache_mnt_show,
...
int ham_init(void)
{
	...
	if (IS_ERR(g_ham_class)) {
		...
		pr_err("failed to init HAM class_stub\n");
