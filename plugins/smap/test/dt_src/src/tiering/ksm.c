...
#include "ksm.h"
  ...
  unsigned long vma_start_pgoff(struct vm_area_struct *v)
{
	return v->vm_pgoff;
}

  unsigned long vma_last_pgoff(struct vm_area_struct *v)
{
	return v->vm_pgoff + vma_pages(v) - 1;
}

  unsigned long avc_start_pgoff(struct anon_vma_chain *avc)
{
	return vma_start_pgoff(avc->vma);
}

  unsigned long avc_last_pgoff(struct anon_vma_chain *avc)
{
...
INTERVAL_TREE_DEFINE(struct vm_area_struct, shared.rb, unsigned long,
  ...
  void *folio_raw_mapping(struct folio *folio)
{
  ...
  struct ksm_stable_node *folio_stable_node(struct folio *folio)
{
