...
struct multi_migrate_struct {
 ...
 struct multi_migrate_struct mig[MAX_NR_MIGRATE_THREADS];
...
void set_remote_migrate_mode(unsigned int mode)
{
 ...
 int smap_check_huge_page_for_migration(struct page *page, pid_t pid)
{
 ...
 int smap_add_page_for_migration(struct page *page, struct folio **folios,
 ...
 int thread_fn(void *data)
{
  ...
  void clear_mig_folios(unsigned int clear_idx)
{
	unsigned int i;
 ...
 void cal_thread_time(ktime_t *start_time, ktime_t *end_time,
			    ktime_t thread_stime, ktime_t thread_etime)
{
 ...
 int init_mig(unsigned int nr_threads, unsigned int nr_folios,
		    int to_node)
{
	unsigned int i;
 ...
 void put_folios(struct folio **folios, unsigned int nr_folios)
{
	unsigned int i;
			   ...
			   unsigned int nr_folios, int to_node)
{
 ...
 int smap_isolate_and_migrate_folios(struct folio **folios,
					   unsigned int nr_folios,
			  ...
			  int to_node, enum smap_migrate_type type)
{
  ...
  void refresh_nodes_nr_free(void)
{
	int i;
 ...
 */
 int find_node_to_migrate_rr(int nid, int *out_nid)
{
	int i, temp_nid;
 ...
 int smap_add_page_for_migrate_back(u64 pa,
					  ...
					  int dest_nid, bool migrate_all)
{
 ...
 bool check_addr_range_valid(struct migrate_back_subtask *task)
{
	__u64 pa;
...
void smap_handle_migrate_back_subtask(struct migrate_back_subtask *task)
{
 ...
 void process_pages_for_migration(struct migrate_back_subtask *task,
					struct folio ***migrate_folios,
					unsigned int *mig_pages_cnt,
...
void smap_handle_migrate_back_subtask_4k(struct migrate_back_subtask *task)
{
 ...
 unsigned int smu_migrate(struct folio **folios, unsigned int nr_folios,
...
size_t nr_abnormal[NR_ABNORMAL] = { 0 };
  ...
  bool is_filter_anon(struct page *page)
{
	if (PageHuge(page)) {
		return false;
	}
 ...
 */
 void collect_one_folio(int idx, struct mig_list *mig_list,
 ...
 */
 int migrate_one_batch(int idx, struct migrate_msg *msg,
 ...
 */
 int process_mig_entry(int idx, struct migrate_msg *msg,
...
int do_migrate(struct migrate_msg *msg, struct mig_list *mig_list)
{
 ...
 int smap_pre_migrate_range(struct folio **folios,
				  unsigned int *nr_folios,
				  ...
				  unsigned long end_pfn)
{
 ...
 unsigned int smap_migrate_range(int nid, u64 start_pa, u64 end_pa)
{
...
struct folio *alloc_demote_page(struct folio *folio, unsigned long node)
{
 ...
 bool check_subtask_range(struct migrate_back_task *task,
				unsigned long pfn)
{
...
bool is_folio_in_migrate_back_range(struct folio *folio)
{
  ...
  bool mig_back_filter(struct folio *folio)
{
