...
#define pr_fmt(fmt) "SMAP_ACPI_helper: " fmt
 ...
 enum acpi_sub_type acpi_get_subtable_type(char *id)
{
...
acpi_get_subtable_header_length(struct acpi_subtable_entry *entry)
{
 ...
 unsigned long acpi_get_entry_type(struct acpi_subtable_entry *entry)
{
	switch (entry->type) {
	case SUBTABLE_COMMON:
 ...
 unsigned long acpi_get_entry_length(struct acpi_subtable_entry *entry)
{
	switch (entry->type) {
	case SUBTABLE_COMMON:
