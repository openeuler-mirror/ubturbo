...
struct mem_info acpi_mem = { .mem = LIST_HEAD_INIT(acpi_mem.mem) };
 struct node_mem acpi_mem_cached[SMAP_MAX_NUMNODES];
...
bool is_paddr_local(u64 pa)
{
 ...
 void update_nr_local_numa(int node)
{
 ...
 int acpi_table_build_mem(struct acpi_subtable_header *header)
{
	int node;
	u32 hotpluggable;
 ...
 int acpi_parse_memory_affinity(union acpi_subtable_headers *header,
				      const unsigned long end)
{
 ...
 int acpi_parse_gicc_affinity(union acpi_subtable_headers *header,
				    const unsigned long end)
{
...
void print_acpi_mem(void)
{
 ...
 void merge_acpi_mem_segments(void)
{
