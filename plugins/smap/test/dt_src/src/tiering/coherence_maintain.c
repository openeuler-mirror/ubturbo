...
void ham_flush_tlb(struct mm_struct *mm)
{
 ...
 int modify_hugetlb_prot(pte_t *pte, unsigned long hmask __always_unused,
			       unsigned long addr,
			       unsigned long next __always_unused,
			       struct mm_walk *walk)
{
	struct modify_info *info = (struct modify_info *)walk->private_data;
...
exit_with_taskput:
 ...
 int hugetlb_entry_collect_pfn(pte_t *pte,
						   ...
						   unsigned long end)
{
 ...
 int modify_single_hugetlb_prot(pte_t *pte,
				      unsigned long hmask __always_unused,
				      unsigned long addr,
				      unsigned long next __always_unused,
				      struct mm_walk *walk)
