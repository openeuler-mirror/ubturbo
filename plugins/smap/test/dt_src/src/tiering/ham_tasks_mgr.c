...
#define pr_fmt(fmt) "HAM: " fmt
 rwlock_t g_tasklock;
 struct ham_migrate_task g_tasklist[HAM_MAX_TASK];

 void reset_migrate_task(struct ham_migrate_task *mig_task)
{
	unsigned int i;
 ...
 void release_migrate_task_inner(struct ham_migrate_task *mig_task)
{
	unsigned int i, j;
