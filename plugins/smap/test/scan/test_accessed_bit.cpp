...
TEST_F(AccessedBitTest, hva_to_hpa_hugetlb)
{
    ...
    MOCKER(huge_page_size).stubs().will(returnValue(PMD_SIZE));
...
TEST_F(AccessedBitTest, hva_to_hpa_hugetlb_two)
{
    ...
    MOCKER(huge_page_size).stubs().will(returnValue(PAGE_SIZE));
...
TEST_F(AccessedBitTest, scan_accessed_bit_forward_mm_fail)
{
    ...
    int ret = scan_accessed_bit_forward_mm(&ap, PAGE_SIZE);
    EXPECT_EQ(-EINVAL, ret);
}
...
TEST_F(AccessedBitTest, scan_accessed_bit_forward_mm_invalid_page)
{
    ...
    ap.pid = 1;
    ap.type = NORMAL_SCAN;
    int ret = scan_accessed_bit_forward_mm(&ap, PMD_SIZE);
    EXPECT_EQ(-EINVAL, ret);
}
...
TEST_F(AccessedBitTest, ActcDataAddFastNumaNoNode)
{
    ...
    actc_data_add_fast(0x1000, PAGE_SIZE);
}

TEST_F(AccessedBitTest, ActcDataAddFastCalcAcpiSuccess)
{
    /* 节点页数表默认为 0，索引检查应在频次更新前返回。 */
...
{
    MOCKER(get_numa_id_by_paddr).stubs().will(returnValue(NUMA_NO_NODE));
    ham_actc_data_add(1, 0x1000, PAGE_SIZE);
}
...
TEST_F(AccessedBitTest, ScanAccessedBitForwardVmInvalidPageSize)
{
    ...
    ap.pid = 1;
    ap.type = NORMAL_SCAN;
    int ret = scan_accessed_bit_forward_vm(&ap, PAGE_SIZE);
    EXPECT_EQ(-EINVAL, ret);
}
...
TEST_F(AccessedBitTest, ScanAccessedBitForwardVmNoScan)
{
    ...
    ap.pid = 1;
    ap.type = NO_SCAN;
    int ret = scan_accessed_bit_forward_vm(&ap, PMD_SIZE);
    EXPECT_EQ(0, ret);
}
...
TEST_F(AccessedBitTest, FillPteVaPaddrZero)
{
    ...
    pte.pte = 0;
    struct mm_walk walk;
    struct pte_walk pw = { .index = 0, .page_size = PAGE_SIZE };
    walk.private_data = &pw;
    int ret = fill_pte_va(&pte, 0x1000, 0x2000, &walk);
    EXPECT_EQ(0, ret);
}
...
TEST_F(AccessedBitTest, ProcessMemslotPages2M)
{
    ...
    int ret = process_memslot_pages(&kvm, &memslot, &ap, PMD_SIZE);
    EXPECT_EQ(0, ret);
    GlobalMockObject::verify();
}
...
TEST_F(AccessedBitTest, ProcessMemslotPages4K)
{
    ...
    MOCKER(smap_kvm_pgtable_stage2_mkold_range).stubs().will(returnValue(0));
    int ret = process_memslot_pages(&kvm, &memslot, &ap, PAGE_SIZE);
    EXPECT_EQ(0, ret);
    GlobalMockObject::verify();
}
