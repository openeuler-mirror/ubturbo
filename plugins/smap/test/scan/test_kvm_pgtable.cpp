...
{
    /* pte含XN位 -> false */
    /* KVM_PTE_LEAF_ATTR_HI_S2_XN = BIT(54) = 1ULL << 54 */
    kvm_pte_t pte = (1ULL << 54);
    bool ret = stage2_pte_executable(pte);
    EXPECT_EQ(false, ret);
}
...
TEST_F(KvmPgTableTest, MkoldRangeAttrClrSet)
{
    ...
    /* KVM_PTE_LEAF_ATTR_LO_S2_AF = BIT(10) = 1UL << 10 */
    /* KVM_PTE_LEAF_ATTR_LO = GENMASK(11,2), KVM_PTE_LEAF_ATTR_HI = GENMASK(63,50) */
    /* attr_clr = BIT(10) & (GENMASK(11,2) | GENMASK(63,50)) = non-zero */
    struct kvm_pgtable pgt = {};
    struct smap_stage2_range_mkold_data data = {};
    data.on_pte_young = stub_on_pte_young;
    data.on_hole = stub_on_hole;
