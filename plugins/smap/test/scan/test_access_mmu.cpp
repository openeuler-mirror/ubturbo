...
TEST_F(AccessMMUTest, CalcVaddrAcidx)
{
    ...
    info.segs[0].start = 0;
    info.segs[0].end = info.segs[0].start + PMD_SIZE;
    info.segs[0].hugepages = 1;

    info.segs[1].start = 0x100000000ULL;
    info.segs[1].end = info.segs[1].start + 2 * PMD_SIZE;
    info.segs[1].hugepages = 2;

    vaddr = info.segs[1].start + PMD_SIZE;
    ret = calc_vaddr_acidx(vaddr, &info, &acidx);
    EXPECT_EQ(0, ret);
    EXPECT_EQ(2, acidx);

    vaddr = info.segs[1].start + 2 * PMD_SIZE;
    ret = calc_vaddr_acidx(vaddr, &info, &acidx);
