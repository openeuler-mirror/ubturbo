 ...
 * Description: smap5.0 migrate_back_ioctl module ut code
 */
...
{
    int ret = smu_mig_open(NULL, NULL);
    EXPECT_EQ(0, ret);
}
...
{
    int ret = smu_mig_ioctl_release(NULL, NULL);
    EXPECT_EQ(0, ret);
}
...
#define SMAP_UNKNOWN_MIGRATE _IOW(SMAP_MIGRATE_MAGIC, 6, int)
...
TEST_F(PidIoctlTest, SmapIoctl)
{
    ...
    ret = smu_mig_ioctl(NULL, cmd, arg);
    EXPECT_EQ(-EINVAL, ret);
...
extern "C" void migrate_dev_exit(void);
