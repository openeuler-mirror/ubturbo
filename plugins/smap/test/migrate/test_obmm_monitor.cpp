...
// Tests for the SMAP monitor of OBMM devices.
#include "smap_msg.h"

extern list_head remote_ram_list;
class ObmmDevTest : public ::testing::Test {
protected:
    void SetUp() override
    {
...
TEST_F(ObmmDevTest, free_remote_ram_dt)
{
...
TEST_F(ObmmDevTest, insert_remote_rams_dt)
{
    int ret = 0;
    u64 pa_start = 0;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_One)
{
    int ret = 0;
    u64 pa_start = 0;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Two)
{
    int ret = 0;
    u64 pa_start = 0;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Three)
{
    int ret = 0;
    u64 pa_start = 0;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Four)
{
    int ret = 0;
    u64 pa_start = 2;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Five)
{
    int ret = 0;
    u64 pa_start = 1;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Six)
{
    int ret = 0;
    u64 pa_start = 1;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Seven)
{
    int ret = 0;
    u64 pa_start = 1;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Eight)
{
    int ret = 0;
    u64 pa_start = 1;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, insert_remote_rams_dt_Nine)
{
    int ret = 0;
    u64 pa_start = 1;
    u64 pa_end = 3;
...
TEST_F(ObmmDevTest, walk_system_ram_remote_range_dt)
{
...
TEST_F(ObmmDevTest, update_obmm_dev_dt)
{
...
TEST_F(ObmmDevTest, is_import_shmdev_dt)
{
...
TEST_F(ObmmDevTest, extract_hex_content_dt)
{
    struct file filp;
    char buf[6] = "0x100";
    u64 content;
...
TEST_F(ObmmDevTest, fill_obmmdev_dt)
{
...
TEST_F(ObmmDevTest, find_range_by_memid_dt)
{
    u64 start;
    u64 end;
...
TEST_F(ObmmDevTest, MergeRamSegmentsEmptyList)
{
...
TEST_F(ObmmDevTest, MergeRamSegmentsSingleEntry)
{
...
TEST_F(ObmmDevTest, MergeRamSegmentsAdjacentSameNid)
{
...
TEST_F(ObmmDevTest, MergeRamSegmentsGapBetweenSegments)
{
    struct ram_segment *seg1 = (struct ram_segment *)kmalloc(sizeof(*seg1), GFP_KERNEL);
    struct ram_segment *seg2 = (struct ram_segment *)kmalloc(sizeof(*seg2), GFP_KERNEL);
    ASSERT_NE(nullptr, seg1);
...
TEST_F(ObmmDevTest, MergeRamSegmentsDifferentNid)
{
    struct ram_segment *seg1 = (struct ram_segment *)kmalloc(sizeof(*seg1), GFP_KERNEL);
    struct ram_segment *seg2 = (struct ram_segment *)kmalloc(sizeof(*seg2), GFP_KERNEL);
    ASSERT_NE(nullptr, seg1);
...
TEST_F(ObmmDevTest, MergeRamSegmentsMultipleMerges)
{
    struct ram_segment *seg1 = (struct ram_segment *)kmalloc(sizeof(*seg1), GFP_KERNEL);
    struct ram_segment *seg2 = (struct ram_segment *)kmalloc(sizeof(*seg2), GFP_KERNEL);
...
TEST_F(ObmmDevTest, UpdateResourceNullResource)
{
    LIST_HEAD(head);
...
TEST_F(ObmmDevTest, UpdateResourceNullArg)
{
    struct resource res = {};
...
TEST_F(ObmmDevTest, UpdateResourceSysramDriverManagedSuccess)
{
    struct resource res;
...
TEST_F(ObmmDevTest, UpdateResourceSysramInsertFail)
{
    struct resource res;
    res.flags = IORESOURCE_SYSRAM_DRIVER_MANAGED;
    res.start = 0x100000;
...
TEST_F(ObmmDevTest, UpdateResourceNonSysram)
{
    struct resource res;
...
TEST_F(ObmmDevTest, InitObmmDevSuccess)
{
    free_obmm_dev();
    ASSERT_TRUE(list_empty(&obmm_dev.list));
...
TEST_F(ObmmDevTest, InitObmmDevKzallocFail)
{
    free_obmm_dev();
    ASSERT_TRUE(list_empty(&obmm_dev.list));
...
TEST_F(ObmmDevTest, CleanObmmDevEmptyList)
{
    free_obmm_dev();
    ASSERT_TRUE(list_empty(&obmm_dev.list));
    clean_obmm_dev();
...
TEST_F(ObmmDevTest, CleanObmmDevRemoveStale)
{
    free_obmm_dev();
...
TEST_F(ObmmDevTest, CleanObmmDevAllSeqMatch)
{
    free_obmm_dev();
...
TEST_F(ObmmDevTest, IncObmmDevSeqEmptyList)
{
    free_obmm_dev();
    ASSERT_TRUE(list_empty(&obmm_dev.list));
    inc_obmm_dev_seq();
...
TEST_F(ObmmDevTest, IncObmmDevSeqIncrement)
{
    free_obmm_dev();
...
TEST_F(ObmmDevTest, IterateObmmDevDirOpenFail)
{
...
TEST_F(ObmmDevTest, IterateObmmDevDirIterateFail)
{
    struct file filp;
...
TEST_F(ObmmDevTest, IterateObmmDevDirSuccess)
{
    struct file filp;
    MOCKER(filp_open).stubs().will(returnValue(&filp));
    MOCKER(IS_ERR).stubs().will(returnValue(false));
...
TEST_F(ObmmDevTest, IterateObmmDevEmptyListInitFail)
{
    free_obmm_dev();
    ASSERT_TRUE(list_empty(&obmm_dev.list));
...
TEST_F(ObmmDevTest, IterateObmmDevEmptyListInitSuccess)
{
    free_obmm_dev();
    ASSERT_TRUE(list_empty(&obmm_dev.list));
...
TEST_F(ObmmDevTest, IterateObmmDevExistingListIterateFail)
{
    free_obmm_dev();
    struct memid_range *mr = (struct memid_range *)kzalloc(sizeof(*mr), GFP_KERNEL);
    ASSERT_NE(nullptr, mr);
...
TEST_F(ObmmDevTest, IterateObmmDevWithExistingList)
{
    free_obmm_dev();
    struct memid_range *mr = (struct memid_range *)kzalloc(sizeof(*mr), GFP_KERNEL);
    ASSERT_NE(nullptr, mr);
...
TEST_F(ObmmDevTest, FindRangeByMemidInvalidRange)
{
    u64 start, end;
    free_obmm_dev();
...
TEST_F(ObmmDevTest, FindRangeByMemidStartEqualsEnd)
{
    u64 start, end;
    free_obmm_dev();
...
TEST_F(ObmmDevTest, FillObmmdevMatchingName)
{
    struct read_obmm_callback call;
    call.ret = 1;
...
TEST_F(ObmmDevTest, FillObmmdevSscanfFail)
{
    struct read_obmm_callback call;
    call.ret = 1;
