			      ...
			      struct mig_batch *batch, struct mig_stats *stats)
{
	...
	flag = smap_add_page_for_migration(p_page, batch->folios, &batch->nr_folios,
					  mig_list[idx].pid, MPOL_MF_MOVE_ALL);
	if (!flag) {
			     ...
			     struct mig_stats *stats)
{
	...
	for (j = folio_index; j < mig_list[idx].nr && batch.cnt < nr_this_migrate;
	     j++, batch.cnt++)
	...
	mig_list[idx].failed_pre_migrated_nr += nr_this_migrate - batch.tmp_pre_nr;
	...
	mig_list[idx].failed_mig_nr +=
		smu_migrate(batch.folios, batch.nr_folios, mig_list[idx].to, &msg->mul_mig);
	...
	if (mig_list[idx].failed_mig_nr)
		pr_debug("[%d]: migrate failed, pre_migrate_num: %d, failed_num: %llu\n",
			 idx, stats->pre_migrate_num, mig_list[idx].failed_mig_nr);

	pr_debug("[%d]: mig_num %d, pre_migrate_num %d, failed_num %llu\n",
		 idx, stats->mig_num, stats->pre_migrate_num, mig_list[idx].failed_mig_nr);
