				     ...
				     u32 mapping_offset)
{
	...
	page_count = get_node_page_count(nid);
	if (!page_count) {
		*actc_len = 0;
		return;
	}
	...
	while (len_cnt < ap->page_num[nid]) {
		...
		if (acidx >= bm_len)
			break;
		if (unlikely(acidx >= page_count)) {
			pr_warn_ratelimited(
				"exceeds total page amount: %llu when lookup access index: %zu on node: %d\n",
				page_count, acidx, nid);
			break;
		}
		...
		if (ret) {
			pr_warn_ratelimited(
				"acidx %zu to paddr failed on node %d\n", acidx,
				nid);
			acidx++;
			continue;
		}
		...
		/* VM 场景 guest RAM 经 memfd/hugetlbfs 后端分配，天然非匿名，
		 * is_file_or_shared_page() 会误判全部白名单；VM 暂不标记白名单。 */
		if (ap->pid_type != SMAP_PID_VM &&
		    is_file_or_shared_page(page))
...
{
	if (tmp->l1_node >= 0) {
		tmp->len[L1] = get_node_page_count(tmp->l1_node);
		if (init_ham_pid_memory(tmp, L1))
			return -ENOMEM;
	}
	if (tmp->l2_node >= 0) {
		tmp->len[L2] = get_node_page_count(tmp->l2_node);
...
int access_walk_pagemap_prepare(struct access_pid *ap)
{
	...
	copy_node_page_count(nodes_page_count);
