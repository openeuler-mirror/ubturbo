...
#undef pr_fmt
#define pr_fmt(fmt) "SMAP_migrate_dev: " fmt
			  ...
			  unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != SMAP_MIGRATE_MAGIC)
		return rc;
	switch (cmd) {
	case SMAP_MIGRATE_BACK:
		return smap_migrate_back_ioctl(argp);
...
{
	int rc = alloc_chrdev_region(&mig_dev, DEVICE_BASE_MINOR,
				     DEVICE_MINOR_COUNT,
				     SMAP_MIG_DEV);
	if (rc < 0) {
...
err_cdev:
	unregister_chrdev_region(mig_dev, DEVICE_MINOR_COUNT);
	return rc;
}
