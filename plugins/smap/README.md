...
1. 冷热数据识别
   ...
   如上图所示，冷热数据识别由`smap.ko`完成：

   1. 模块初始化时创建`/dev/smap_scan_dev`，并初始化软件扫描；设置`enable_hist=1`时同时初始化硬件判热子系统（仅鲲鹏950机型支持）；
   2. 上层管理模块通过扫描设备下发PID管理、页表扫描和频次查询等命令；
   3. 扫描子系统根据配置执行软件扫描或硬件扫描，并将结果返回用户态；
